<template>
  <el-card class="box-card">
    <template #header>
      <div><span style="font-size: 20px">更新角色</span></div>
    </template>
    <div class="table">
...
import { getRole, getRoleTree, putUpdateRole } from "@/api/http";
...
onMounted(() => {
  const id = localStorage.getItem("/edit/updateRole");
  getRole(id).then((res) => {
    if (res.code === "200") {
      role.value = res.data;
    }
...
const onSubmit = async () => {
  console.log(JSON.stringify(role.value.valueOf()))
  putUpdateRole(JSON.stringify(role.value.valueOf())).then((res) => {
    if (res.code === "200") {
